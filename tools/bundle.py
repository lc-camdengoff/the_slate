#!/usr/bin/env python3
"""Unpack / repack the app source inside storyboard.html.

storyboard.html is a self-extracting bundle: a small unpacker, a
...
BUNDLE = os.path.join(ROOT, 'storyboard.html')
...
def read_bundle():
    ...
    if not match:
        sys.exit('error: no __bundler/template block in storyboard.html')
    return bundle, match
...
def pack():
    ...
    with open(BUNDLE, 'w', encoding='utf-8') as fh:
        fh.write(updated)
    print('packed %d bytes -> storyboard.html%s' % (
...
def check():
    ...
    if match.group(2) != encode(template):
        sys.exit('error: storyboard.html is stale — run: python3 tools/bundle.py pack')
    print('ok: storyboard.html matches src/template.html')
